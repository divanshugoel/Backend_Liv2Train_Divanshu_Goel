...
public class RestResponseEntityExceptionHandler extends ResponseEntityExceptionHandler {
	...
	/**
	 * handleAppException is a method to customize
	 * the response for AppException.
	...
	protected ResponseEntity<Object> handleAppException(AppException ex, WebRequest request) {
		ErrorResponseDTO errorResponseDTO = new ErrorResponseDTO(new Date(),ex.getErrorCode(),
				 ex.getMessage());
		return new ResponseEntity<>(errorResponseDTO, HttpStatus.INTERNAL_SERVER_ERROR);
	}

	/**
	 * handleMethodArgumentNotValid is a method to customize
